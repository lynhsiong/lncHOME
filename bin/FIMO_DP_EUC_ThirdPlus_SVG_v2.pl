...
$mp->start and next;
     ...
     if ($ele=~/^\//){
      ...
      if ( -s $infimo3rd ){
         ...
         if ( not $? ) {
         `perl $mybindir/motif_block_merge_edgeR_3rd.pl 10 200 $fimo_Hs $infimo3rds $prefixH`;
         }
         if ( not $? ) {
         `perl $mybindir/motif_block_merge_edgeR_3rd.pl 10 200 $fimo_Ms $infimo3rds $prefixM`;
         }
...
`$mybindir/dynprogress3 $outputfile_H 0.01 -t 0.2` if ( not $? );
...
`$mybindir/dynprogress3 $outputfile_M 0.01 -t 0.2` if ( not $? );
