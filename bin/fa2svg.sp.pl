...
@code_arr = ("HUM","MOU","COW","OPS","CHC","LIZ","FRG","ZBF");
...
for (2..$#code_arr){
    $mysp=$code_arr[$_]; $HFH=$mysp."HFH"; $MFH=$mysp."MFH";
    $HsynF=`find $syngeneDB -maxdepth 1 -type f -name "HUM2$mysp.genepair.syn"`; chomp $HsynF;
    $MsynF=`find $syngeneDB -maxdepth 1 -type f -name "MOU2$mysp.genepair.syn"`; chomp $MsynF;
    open $HFH, "<$HsynF"; @hArr=<$HFH>; close $HFH;
    open $MFH, "<$MsynF"; @mArr=<$MFH>; close $MFH;
    push @fArr, @hArr; push @fArr, @mArr;
    for (@fArr){
        chomp;
        @synFilds=split("\t", $_);
        push @{$SYN{$mysp."_".$synFilds[0]}}, $synFilds[1];          #### hum/mou TO 3rd_sp   eg. COW_ENSG* => @{COW_gene1, COW_gene2, ...}
        #push @{$RVSYN{$synFilds[1]}}, $synFilds[0];        #### 3rd_sp TO hum/mou
    }
}

for (0..$#code_arr){
    $myfsp=$code_arr[$_]; $FFH=$myfsp."FH";
    $fsynF=`find $lncPoolDB -maxdepth 1 -type f -name "$myfsp.gen2trx.lst"`; chomp $fsynF; 
    open $FFH, "<$fsynF";
    while (<$FFH>){
        chomp;
        @fsynFilds=split("\t", $_);
        push @{$POOL{$fsynFilds[0]}}, $fsynFilds[1];      ####  gene TO @{trx}
        #$RVPOOL{$fsynFilds[1]}=$fsynFilds[0];             ####  trx TO gene
    }
    close $FFH;
}
...
while ($inf=<INF>){
    ...
    chomp $inf;
    @F=split("\t", $inf);
    $inhash{$F[0]}=$F[1];        #### ENST*** HUM
    $rev_inhash{$F[1]}=$F[0];     #### HUM ENST***
    }
}
...
if ($mode==2){
    ...
    ($NONCODEhgene, $NONCODEmgene, $NONCODEhtrx, $NONCODEmtrx)=read_cand2NONCODE($cand2NONCODE);
    if (($NONCODEhgene=~/ENSG/)&&($NONCODEmgene=~/ENSMUSG/)){
       for (2..$#code_arr){
           $temp_code=$code_arr[$_]; @temp_arr_gene=();   
           if (defined $SYN{$temp_code."_".$NONCODEhgene}){
               push @temp_arr_gene, @{$SYN{$temp_code."_".$NONCODEhgene}};
           }
           if (defined $SYN{$temp_code."_".$NONCODEmgene}){
               push @temp_arr_gene, @{$SYN{$temp_code."_".$NONCODEmgene}};
           }
           if(scalar(@temp_arr_gene)>0){
              for (@temp_arr_gene){ 
                 push @{$trxhash{$NONCODEhtrx."_".$NONCODEmtrx}{$temp_code}}, @{$POOL{$_}} if defined($POOL{$_});
              }
           }else {
                 $trxhash{$NONCODEhtrx."_".$NONCODEmtrx}{$temp_code}="NOTRD";
           }
       }
    }     
    $rmode="c";        # hum to mou
...
if ( $run_mode <= 1 ){
  foreach $mytempSpCode (keys %othersupport_hash){
   ...
   $tempsplncPool=`find $lncPoolDB -maxdepth 1 -type f -name "$mytempSpCode.lncRNApool.fa"`; chomp $tempsplncPool;
...
&domainSVG( 2479, 10000, \@color, \%sp2domain, \%domain2motif, \%domain2block, \%faLength, "MYSVG", $mysvgout );
...
sub read_cand2NONCODE{
  ...
  if (($hExist eq "yes") && ($mExist eq "yes")){
     return ($hNONgene, $mNONgene, $hNONintrx, $mNONintrx);
  }else {
...
$PairLncH =$dir."/"."lnc".$pid.".HUM.fa";   $PLFH="lncH".$pid;
$PairLncM =$dir."/"."lnc".$pid.".MOU.fa";   $PLFM="lncM".$pid;
...
print $BASH "\n\n";
print $BASH "/Share/home/zhangqf2/tools/meme/bin/meme ",$PairLncFa," -p 5 -nostatus -time 36000 -dna -revcomp -text -mod anr -nmotifs 10 -minw 5 -maxw 30 -maxsites 600 -maxsize 1000000 >",$meme,"\n";       ### ?? meme-chip not considering revcomp (lots of RNA segments containing 1 or 2 motifs), but meme maybe should consider revcomp (only several RNA sequences); purpose of the former is identifying significant (top) motifs, but the latter is obtaining the full landscape of de novo motifs and their (maybe also conserved) revcomp position; PS: fimo should considers both strands.      e.g.   ....atg....atg....cat....cat....   in meme mode, 1 motif 2 diretions (revcomp); 2 motifs 1 direction (no revcomp)
print $BASH "/Share/home/zhangqf2/tools/meme/bin/fimo --verbosity 1 --text ",$meme," ",$PairLncFa," >",$fimo,".pre.denovo","\n"; 
print $BASH "perl $scrdir/motif_update.pl ",$meme," ",$PairLncFa," ",$meme,".update","\n";
print $BASH "/Share/home/zhangqf2/tools/meme/bin/fimo --verbosity 1 --text ",$meme,".update ",$PairLncFa," >",$fimo,".denovo","\n";

print $BASH "/Share/home/zhangqf2/tools/meme/bin/fimo --verbosity 1 --text ",$motif_db,"/HUM.motifpool.meme ",$PairLncH," >",$fimo,".clipNdb","\n";
print $BASH "/Share/home/zhangqf2/tools/meme/bin/fimo --verbosity 1 --text ",$motif_db,"/MOU.motifpool.meme ",$PairLncM," >>",$fimo,".clipNdb","\n";
print $BASH "cat $fimo",".denovo ",$fimo,".clipNdb ", ">$fimo.combined","\n";  

print $BASH "cat ",$fimo,".combined","|awk -F \'\\t\' \'(\$6>=13)&&(\$7<=0.00005){print \$0}\' |sort -k2,2 -k3,3n -k4,4n >",$fimo,".sort.filter","\n";
print $BASH "perl $scrdir/fimo_pair_motif_filtering.pl ", $fimo,".sort.filter 0.00005 >", $fimo,".sort.pair-filter","\n";

print $BASH "perl $scrdir/motif_block_merge_edgeR.pl 4 120 ",$fimo,".sort.pair-filter ","fimo",$pid,".out.sort.p-f.sigwindows", "\n";
...
print $BASH "for cl in $dir/cluster/C*.sort; do if [ -f \"\$cl\" ]; then $scrdir/dynprogress3 \$cl 0.01 -t 0.2; fi; done\n";
print $BASH "\n\n";

print $BASH "DIRdir=\"$dir\"","\n";
print $BASH "SCRdir=\"$scrdir\"","\n";
print $BASH "TRDdir=\"$trddir\"","\n";
print $BASH "MTFdb=\"$motif_db\"","\n";
...
print $BASH '   perl $SCRdir/ClusterSelectionbyDPscore.pl ${DIRdir}/cluster/total.cluster_align 3 ${DIRdir}/cluster/Selected_Align/selected_cluster.out',"\n";
...
print $BASH '   SP=(COW OPS CHC LIZ FRG ZBF)',"\n";
...
print $BASH '       perl $SCRdir/compare_3rd_v2.0.pl ${DIRdir}/cluster/Selected_Align/selected.cluster_align $seq_3rd ${DIRdir}/cluster $MTFdb',"\n";
#print $BASH ' ',"\n";
...
#print $BASH '       done',"\n";
#print $BASH ' ',"\n";
print $BASH '       perl $SCRdir/FIMO_DP_EUC_ThirdPlus_SVG_v2.pl ${DIRdir}/cluster/Selected_Align/selected.cluster_align $seq_3rd ${DIRdir}/cluster',"\n";  
print $BASH '     fi',"\n";
print $BASH '   done',"\n";
#print $BASH ' ',"\n";
#print $BASH 'fi',"\n";
